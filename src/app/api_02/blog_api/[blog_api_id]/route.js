...
export async function GET(request, { params }) {
  ...
  try {
    if (data.length != 0) {
      ...
      return NextResponse.json(
        { result: "id is not found", success: false },
        { status: 404 }
      );
    }
  } catch (err) {
    return NextResponse.json(
      { result: "data is not found ", success: flase },
      { status: 404 }
    );
  }
}
...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    return NextResponse.json(
      { result: "Blog is deleted ", success: true },
      { status: 200 }
    );
  } catch (err) {
    return NextResponse.json(
      { result: "something wrong pls Try again ", success: false },
      { status: 404 }
    );
  }
}
